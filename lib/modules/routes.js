...
var	fs         = require( 'fs'                      ),
	...
	textLoader = require( './text-loader.js'        ),
 ...
 *
 * The args ('filter', 'a=1&b&c=foo') results in the querystring 'filter[a]=1&filter[b]&filter[c]=foo'.
 */
function dictToQuery( dictName, dict ) {
	...
	for ( i = 0, len = parts.length; i < len; i += 1 ) {
		...
		currentQueryParam = dictName + '[' + encodeURIComponent( key ) + ']';
		if ( value ) {
...
function getRequestConfig( cli, args, options, api ) {
	...
	if ( options.method === 'GET' || options.method === 'DELETE' ) {
		 ...
		 */
		dictionaries = [];
		findPrefix( options, 'dict:', function ( dictName, dictData ) {
			if ( ! textLoader.textOptions[ dictName ] ) {
				dictionaries.push({
					name: dictName,
					data: dictData,
				});
		...
		if ( dictionaries.length > 0 ) {
			...
			dictionaries.forEach( function ( dict ) {
				query += dictToQuery( dict.name, dict.data );
			});
		}
