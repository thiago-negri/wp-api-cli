 ...
 */
function getRoute( context, cli, args, options, api ) {
	var	routes = gCliRoutes.commands[ cli.command ].routes,
		matchingRoutes = [],
		argCount = 0;
 ...
 */
function getRequestConfig( context, cli, args, options, api ) {
	var	matchingRoutes = getRoute( context, cli, args, options, api ),
		route,
		routeObj,
		routeArgs,
		endpointArgs = [],
 ...
 */
function commandHandler( context, cli, args, options, api, callback ) {
	var requestConfig = getRequestConfig( context, cli, args, options, api );
...
/**
 * Makes a text to have a minimum length.
 *
 * @param text Text to use.
 * @param length The desired minimum length.
 * @param filler The character to be appended to the text until it has minimun length.
 ...
 */
function handleInfoCommand( context, cli, args, options, api, callback ) {
	var	cmd,
		cmdRoutes,
		keyLength = 20,
		keyFiller = '.';

	if ( ! gCliRoutes.commands.hasOwnProperty( args[0] ) ) {
		return callback( 'Unknown command "' + args[0] + '"' );
	}
	cmd = gCliRoutes.commands[ args[0] ];
	...
	cmdRoutes.forEach( function( route ) {
		...
		if ( pathOptions.length > 0 ) {
			...
			pathOptions.forEach( function ( arg ) {
				if ( gCliRoutes.options[ arg ] !== undefined ) {
					cli.info( '    --' + arg );
				}
			});
		}
		routeObj.endpoints.forEach( function ( endpoint ) {
			...
			for ( key in endpoint.args ) {
				if ( endpoint.args.hasOwnProperty( key ) ) {
					if ( gCliRoutes.options[ key ] !== undefined ) {
...
function load( context, api, callback ) {
	fs.readFile( context.apiDescriptionFile, 'utf8', function ( error, fileContent ) {
		...
		if ( gRoutes ) {
			gCliRoutes.options = extractOptions( gRoutes );
